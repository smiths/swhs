...
$t_\text{melt}^\text{init}$, [$T_W^{\text{Solid}}(t_\text{melt}^\text{init})$,
  $T_P^{\text{Solid}}(t_\text{melt}^\text{init})$]$^T$,
  event\_EndMelt)\\
...
\noindent $E_W$ := EnergyWater($T_W^{\text{Solid}}$) ...\\
\textit{conditional rule with time constraints - or energy equation
  for each zone}
...
\begin{center}
\begin{tabular}{p{2cm} >{\raggedright\arraybackslash}p{5cm} >{\raggedright\arraybackslash}p{2cm} p{3.5cm}}
  \hline
  \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
  \hline
  solve & systFunctT n, $\mathbb{R}$, stateT n, stateBasedFuncT n &
                                                                 $\mathbb{R}$,
                                                                    seqFuncT
                                                                    n & ODE\_BAD\_INPUT, ODE\_MAXSTEP, ODE\_ACCURACY \\
  solve (alternative notation) & $(\mathbb{R}^n \rightarrow \mathbb{R})^n$, $\mathbb{R}$,
          $\mathbb{R}^n$, $\mathbb{R}^n \rightarrow \mathbb{R}$ & $\mathbb{R}$, 
                                                                  $(\mathbb{R}
                                                                  \rightarrow
                                             \mathbb{R})^n$ & ODE\_BAD\_INPUT, ODE\_MAXSTEP, ODE\_ACCURACY \\
  solveNoE & systFunctT n, $\mathbb{R}$, stateT n, $\mathbb{R}$ &
                                                                    seqFuncT
                                                                    n & ODE\_BAD\_INPUT, ODE\_MAXSTEP, ODE\_ACCURACY \\
...
solve($f$, $t_0$, $y_0$, $t_\text{fin}$): 
\begin{itemize}
\item output: $out := t_1, y(t)$ where 
$$y(t) = y_0 + \int_{t_0}^{t_\text{fin}} f(y(s)) ds$$ 
...
\section{MIS of Types} \label{SpecParam}
...
functT = $\mathbb{R} \rightarrow \mathbb{R}$\\
seqFuncT n = (functT)$^n$ = $(\mathbb{R} \rightarrow \mathbb{R})^n$\\
stateT n = $\mathbb{R}^n$\\
stateBasedFuncT n = stateT n $\rightarrow \mathbb{R}$ = $\mathbb{R}^n
\rightarrow \mathbb{R}$\\
systFunctT n = (stateBasedFuncT n)$^n$ $= (\mathbb{R}^n \rightarrow \mathbb{R})^n$\\
