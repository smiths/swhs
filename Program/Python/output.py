def output(params, time, tempW, tempP, eW, eP, eTot, filename):
    ...
    results = zip(time, tempW, tempP, eW, eP, eTot)
    ...
    f.write('\n' + 'Time'.ljust(max_width+1) + 'Twater'.ljust(max_width+1) + 'TPCM'.ljust(max_width+1) +
            'Ewater'.ljust(max_width+1) + 'EPCM'.ljust(max_width+1) + 'Etotal'.ljust(max_width+1) + '\n')
    for t, Tw, Tp, Ew, Ep, Et in zip(time, tempW, tempP, eW, eP, eTot):
